...
fun main() {
    val generatedPeople = PersonGenerator.persons(100)
    //print 10 first people created
    generatedPeople.take(10).forEach{println(it.nice())}
    
    println("")
    println("")
    
    //print out the number of persons whose first name has less than 5 letters, and who live in a city with an odd zip code.
    val filtered = generatedPeople.filter{p -> p.name.split(" ")[0].length < 5 && p.address.zip.toInt() % 2 != 1}
    println(filtered.toList().size)
    
    println("")
    println("")
    
    //group persons by the length of their name, and print out the number of persons in each group
    val groupedMap = generatedPeople.groupBy{p -> p.name.length}
    groupedMap.forEach{key,value -> println(""+ key + " - "+value.size)}

    println("")
    println("")
	
    //make sorted()		todo: why is sorted alredy working=
    val tree = PersonTree()
    generatedPeople.
    ...
    val tree2 = PersonTree()
    generatedPeople.forEach { tree2.insert(it) }
    var sortedT = tree2.sorted()
    var sorted1 = sortedT.take(10)
    var sorted2 = sortedT.drop(10).take(10)
    println(sortedT.take(20).map{p -> p.name}.toList())
    println(merge(sorted1,sorted2).map{p -> p.name}.toList())
    
    //toSeq()
    
    println(tree2.toSeq().take(20).map{p -> p.name}.toList())
...
class PersonTree() {
  ...
  private class Node (val person: Person = Noone) {
      ...
      fun toSeq() : Sequence<Person> {
          if(this@Node != Empty){
              return left.toSeq() + sequenceOf(person) + right.toSeq()
          }
  ...
  fun toSeq() : Sequence<Person> = root.toSeq()

}
fun merge(s1: Sequence<Person>, s2: Sequence<Person>): Sequence<Person> = sequence{
    //return (s1 + s2).sorted()
