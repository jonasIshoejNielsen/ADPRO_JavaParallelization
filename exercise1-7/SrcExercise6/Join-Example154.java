...
class FunList<T> {
  ...
  public int count(Predicate<T> p){
    ...
    int sum = 0;
    while (curr != null){
      sum = (p.test(curr.item))?sum+1:sum;
      curr = curr.next;
    }
    return sum;
  }
  ...
  //Question 3
  public FunList<T> filter(Predicate<T> p){return new FunList<T>( filter (this.first, p));}
  public Node<T> filter(Node<T> node,Predicate<T> p){
    if (node == null) return null;
    if (p.test(node.item)) return filter(node.next, p);
    return new Node<>(node.item, filter(node.next, p));
  }
  //Question 4
  public FunList<T> removeFun(T item){
    return filter(i-> i==item);
  }

  //Question 5
  public FunList<T> flatten(FunList<FunList<T>> xss){
    var res = new FunList<T>();
    var curr = xss.first;
    while (curr!=null){
  ...
  public static <T> FunList<T> flattenFun(FunList<FunList<T>> xss){
    return xss.reduce( new FunList<>(), (acc, a) -> acc.append(a) );
  }

  // Question 7
  public <U> FunList<U> flatMap(Function<T,FunList<U>> f){
    var res = new FunList<U>();
    var curr = this.first;
    while (curr!=null){
      res.append(f.apply(curr.item));
      curr = curr.next;
    }
    return res;
  }
  ...
  public <U> FunList<U> flatMapFun(Function<T,FunList<U>> f){
    return FunList.flattenFun(this.map(f));
  }
  ...
  public FunList<T> scan(BinaryOperator<T> f){
    ...
    int i=0;
    while (curr!=null){
      prev = (prev==null)? curr.item : f.apply(prev, curr.item );
      res.insert(i++, prev);
      curr = curr.next;
    }
    return res;
